...
class HangmanTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label_letters = [self.char_to_index[char] for char in word_list]
        ...
        labels = torch.tensor([-100] + label_letters + [-100]*(self.max_length - len(word_list)-1)).squeeze(0)
...
for epoch in range(num_epochs):
    ...
    if (epoch+1) % save_frequency == 0 or avg_val_loss<prev_val_loss:
        model_save_path = os.path.join(save_dir, f"bert_ce_model_epoch_{epoch+1}.pth")
        checkpoint = {
            'epoch': epoch + 1,
