...
class HangmanEnv:
    ...
    def step(self, letter):
        if letter in self.guessed_letters:
            return self.hidden_word, -1, self.done  # Penalize repeated guesses
...
def train_policy_gradient(train_words, model, optimizer, patience=5000):
    ...
    for episode in range(500000):
        ...
        if episode % 10000 == 0:
            model_save_path = os.path.join(save_dir, f"bert_rl_model_epoch_{episode}.pth")
            checkpoint = {
                'episode': episode,
