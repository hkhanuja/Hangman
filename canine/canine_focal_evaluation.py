...
checkpoint = torch.load('canine/canine_focal/canine_focal_model_epoch_70.pth', map_location=device)  
...
with torch.no_grad():
  for word in tqdm(val_words):
    ...
    while lives>=0 and len(letters_to_be_masked)>0:
        tokenized_word = canine_tokenizer(
          str(word),
          return_tensors="pt",
          truncation=True,
      )
        ...
        for i in range(len(word_list)):
            if word[i] in letters_to_be_masked:
                input_ids[i+1] = canine_tokenizer.mask_token_id
        ...
        max_prob_char = torch.argmax(logits, dim=-1)
        # print(max_prob_char)
...
with open("canine/canine_focal/canine_focal_correct_words.txt", "w") as f:
