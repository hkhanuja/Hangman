...
class BertHangman(nn.Module):
  ...
  def forward(self, input_ids=None, attention_mask=None, guessed_letters=None):
    ...
    mask_token_indices = (input_ids == bert_tokenizer.mask_token_id).nonzero(as_tuple=False)  
...
checkpoint = torch.load('bert_mean_ce/bert_mean_ce_model_epoch_100.pth', map_location=device)  
...
with open("bert_mean_ce/bert_mean_ce_correct_words.txt", "w") as f:
